...
// This Function it will call the function from the AI module to determine
// the next state given the current board Vertically
std::vector<char>& BoardToGrammer::getNextVerticalState() {
  // will call the function from the AI module to determine the next state
  // given the current board
  return (BoardPtr->getNextVertical(VerticalIndex++));
}

// This Function it will call the function from the AI module to determine
// the next state given the current board Horizontally
std::vector<char>& BoardToGrammer::getNextHorizontalState() {
  return (BoardPtr->getNextHorizontal(
      HorizontalIndex++));  // what if reached to 15?
}

// This Function will call the GUI function which will aquire the move
