...
Board::Board() {
  for (int i = 0; i < 26;
       i++)  // i want to create them at insertion of the letter as i dont need
             // the whole 26 char along the game
  {
    BoardMap[(char)('A' + i)] = BoardMask(
...
void Board::setBoard(uint8_t board[15][15]) {
  for (int i = 0; i < 15; ++i) {
    for (int j = 0; j < 15; ++j) {
      if (board[i][j] <= 26 && board[i][j] >= 1)
        this->SetCharPos(board[i][j] + 'A' - 1, i, j);
      else if (board[i][j] <= 126 && board[i][j] >= 101)
        this->SetCharPos(board[i][j] + 'A' - 101, i, j);
    }
  }
}
...
BoardMask Board::getBoardStatus() { return AllCharBoard; }
...
// The Function getNextHorizontal it will call the function from the AI module
// to determine the next state given the current board Horizontally
std::vector<char> &Board::getNextHorizontal(int HorizontalIndex) {
  ...
  for (auto Instance : BoardMap)  // an instance of a letter
  {
    if ((Instance.second & CheckBoard).isEmpty()) {
      CharVect.push_back(
          Instance.first);  //  need to edit the position in the vector
    }
  }
  int mOffsit = HorizontalIndex;
  for (int i = 0; i < 15; i++)  // looping on the col
  {
    for (std::size_t i = 0; i < CharVect.size();
         ++i)  // looking on the char board that has  this bit
    {
      ...
      if (i ==
          CharVect.size() -
              1)  // i reached the end and this offsit is not in any char board
...
// This Function it will call the function from the AI module to determine the
// next state given the current board Vertically
std::vector<char> &Board::getNextVertical(int VerticalIndex) {
  ...
  for (int mOffsit = VerticalIndex * 15; mOffsit < (VerticalIndex * 15 + 15);
       mOffsit++)  // looping on the col
  {
    for (std::size_t i = 0; i < CharVect.size();
         ++i)  // looking on the char board that has  this bit
    {
...
void Board::SetCharPos(char Letter, int Row, int Col) {
  ...
  if (Offsit < 0 || Offsit > (14 + 15 * 14)) return;
  ...
  for (int i = 0; i < 15; i++) {
    ...
    for (int j = 0; j < 15; j++) {
      if (AllCharBoard.getBit(mOffsit)) {
        for (auto Instance : BoardMap)  // an instance of a letter
        {
          if (Instance.second.getBit(mOffsit)) cout << Instance.first << " ";
        }
      } else {
        cout << "0"
             << " ";
...
// Destructor
Board::~Board() { Board::BoardInst_ = nullptr; }
