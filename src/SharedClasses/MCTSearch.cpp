...
MCTSearch::MCTSearch(list<Move> Moves) { this->Root = new MCTreeNode(Moves); }
...
MCTreeNode* MCTSearch::SelectBestMove(MCTreeNode* Parent) {
  ...
  for (int i = 0; i < Parent->Children.size(); i++) {
    if (Parent->Children[i]->getNumberOfVisits() == 0) {
      ...
      CurrentScore = Parent->Children[i]->getMoveScore() +
                     2 * cp *
                         log((2 * Parent->getNumberOfVisits()) /
                             Parent->Children[i]->getNumberOfVisits());
    }
...
int MCTSearch::endGameMCTS() {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    if (SecondMove == NULL) FirstMove->setNonExpandable();

    // expand from depth=2 if possible and if not we should mark node selected
    // from depth=2 expandable=false then select the second best option  to
    // expand it, until all are not expandable then the whole tree from node
    // selected at depth=1 is not expanable and therefore mark it as non
    // expanable
    SecondMove->expandEndGame();

    // We will not select again but instead perform simulation on the last
    // expanded node as it is the newest node to the path to the tree
    ...
    float MoveDifference =
        FirstMove->getMoveScore() + MySecondMove - SecondMove->getMoveScore();
...
int MCTSearch::midGameMCTS() {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    // call function that is called expandmidgame on FirstMove Node that will
    // create new node and add it to children give it the new depth and value.
    // If list of available moves is empty then mark this node as un expandable
    ...
    // Finally the value will be back propagated to the d=1 node to calculate
    // the move score For now we will be saving the maximum score in the branch
