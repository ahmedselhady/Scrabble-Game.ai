...
AI_MODE::AI_MODE() { this->gaddag_instance = GameBrain::__get_gaddag(); }
...
Move *AI_MODE::doWork() {
  ...
  std::future<std::vector<char>> rack_generation_thread =
      std::async(&OpponentRack::RackGenerator, &opponentRackGenerator,
                 *this->bagReference);
  ...
  // gameEvaluator::__get_Instance()->evaluateMovesHuristic(listOfMoves,
  //                                                END_GAME_MODE);

  // when done with those: use game evaluator to limit the moves to 23 moves
  // only:
