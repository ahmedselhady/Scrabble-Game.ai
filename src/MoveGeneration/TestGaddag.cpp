...
int main() {
  ...
  //         std::tolower); std::transform(move2.begin(), move2.end(),
  //         move2.begin(), std::tolower); if(move == move2){
  //             break;
  //         }
  //         MoveFile>>temp666;
  //         MoveFile>>temp666;
  ...
  cout << "malpractitioner" << (bool)Gen->checkWordDict("malpractitioner")
       << endl;
  cout << "malpractitionerx" << (bool)Gen->checkWordDict("malpractitionerx")
       << endl;
  cout << "xmalpractitioner" << (bool)Gen->checkWordDict("xmalpractitioner")
       << endl;
  cout << "malpracvvtitioner" << (bool)Gen->checkWordDict("malpracvvtitioner")
       << endl;
  ...
  for (std::list<Move>::iterator it = moves.begin(); it != moves.end(); it++) {
    MovesFile << it->word << " " << it->horizontal << " "
              << (int)it->startPosition.ROW << " " << (int)it->startPosition.COL
              << " " << it->moveScore << "\n";
    if (it->word == "azo") {
      cout << endl
           << it->word << endl
           << it->horizontal << it->startPosition.ROW << it->startPosition.COL
           << it->moveScore;
    }
  }
  MovesFile.close();
  int movesCount = moves.size();
  //  simple test case i have a vertical word apple and 1 intersecition and 1 x3
  //  score and x2 letter score
...
/*
    How To Use The GADDAG DataStructure (DS):
        1- Create an Object From the "LoadGaddag" Class Used in The Creation of
   The DataStructure. 2- Call "constructGaddag()" Function Found in "LoadGaddag"
   class Responsible For The Creation of DS it returns a Pointer of Type "Node*"
   Class which will have the address the Root of the GADDAG TREE. 3- Now You
   Have The GADDAG ;) Enjoy.

    How To Use The "WordGenerate" Class Found inside "MoveGenerate.h and .cpp"
   Responsible for Possible Moves Given a Board State and a Rack: 1- Create an
   object of from "WordGenerate" class and pass to it in the constructor the
   Board as "BoardToGrammer*" and the GADDAG Root Tree "Node*". 2- To Set The
   DESIRED Rack For Generations, Call The Function "countTilesRack(&Rack)" and
   pass a "vector<char>*rackTiles" as an argument inside "WordGenerate" Class,
   This Sets The Rack Tiles From the Inside. 3- You Can Also Set The (Board
   State) using The "setBoardState(BoardToGrammer*board)" given a
   "BoardToGrammer*" For Different Board State Configurations. 4- Now You Have
   SET UP the Needed Environment for this Class,  NOW ITS TIME FOR MOVES !!? 5-
   There Are Two Methods Depending On Board State: 1- emptyBoardMoves() --> 1st
   State of The Board Where No one Played Any Moves. 2- generateWords() -->
   Whereever There are WORDS on The Board.
                                        -------> Choose Wisely and Call The
   Appropriate One. 7- Now After All of This you are One Step Away from Your
   GOAL : MOVES, Just Call "allMoves()" to Get a List of Moves. 8- New
   Functionalities May Be ADDED For Better, Optimized and Fast Possible Move
   Generations --> STAY TUNED!!.
*/

/* TESTED :
        1- EmptyBoard Generation. Using Different Tiles in Rack including
   BLANKS.
*/
