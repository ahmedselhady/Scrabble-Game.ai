...
// Function readDictFile it reads dict. files which contains valid words given
// filename.
void LoadGaddag::readDictFile(vector<string> &dictGaddagWords,
                              const char *fileName) {
  string word;
  ...
  if (myFile.is_open()) {
    ...
    while (myFile >> word) {
      // Each word will be multipied by number of its characters. ex: "cat" will
      // have three entries in GADDAG.
...
// Function constructGaddag it builds Gaddag trie and returns compressed trie
// node.
Node *LoadGaddag::constructGaddag() {
  ...
  if ((stat(GADDAG_FILE_NAME, &buffer) == 0)) {
    ...
    return (Node *)&gaddagRootNode[0];  // ROOT .. return address of the first
                                        // value in compressed byte offset array
                                        // casted in a Node type.
  }

  // Gaddag Root Construction.
  vector<string> dictGaddagWords;  // GADDAG words from given dict. file.
  vector<LoadNode *> gaddagNodes;  // store all nodes to then output a
                                   // compressed structure then.
  readDictFile(dictGaddagWords, DICT_FILE_NAME);
  for (int index = 0; index < dictGaddagWords.size(); ++index) {
    Root.insertGaddagWord(
        dictGaddagWords[index]);  // inserting word into GADDAG.
  }
...
// The function CompressGaddag it compresses the trie into an array of nodes
// using byte offset techinque.
void LoadGaddag::compressGaddag(unsigned int *&gaddagRootNode,
                                vector<LoadNode *> &gaddagNodes) {
  ...
  /* info in compressedLetter : 1- letter after compression.
                                ...
                                */
  unsigned int address;  // just the distance between the parent and child that
                         // will be used
  ...
  for (int index = 0; index < gaddagNodes.size(); ++index) {
    ...
    if (address != 0) {  // Byte Index.
      address -= index;  // as all counts(addresses) are increased by one to
                         // achieve a right indexed offset.
    }
