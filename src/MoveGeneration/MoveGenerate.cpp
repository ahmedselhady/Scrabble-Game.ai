...
/*
This Function iterates on each sqaure in the board and performing the algo.
Taking the board 2-Dimensions into Consideration. and it iterates on each sqaure
in the board and performing the algo.
*/
void WordGenerate::generateWords() {
  ...
  moves.clear();  // clear all moves. Later on paramters(BOOL) can be passsed to
                  // shape the needed return.
...
bool WordGenerate::isAnchor(int row, int col) {
  // NOTE: GADDAG algo. works from the left to right. it can be reversed but
  // also need reverse of the structure itself DAGGAD.

  // what is an anchor square:
  /*
     1- if it has a letter on the square and its right sqaure doesn't.
      this means that "log" anchor's is 'g'. beacuse no need of 'o' or 'l' as
     anchors giving same repeated results. 2- if it doesnot have a letter but it
     should at least have an opposite cross set to its current direction. this
     means that if a vertical word directly above a square i can attach a word
     to this square but hoirz.

  */

  if (board->hasaTile(row, col) &&
      (currDirection == maxBorder - 1 ||
       !board->hasaTile(row + rowOffset, col + colOffset))) {
  ...
  return false;
}  // is this square given row and col an anchor sqaure? Offsets used to make
   // the function works in 2D (vert./horiz.)

// This Function it's important to stop right before a letter found while
// travesring the board leftward generating possible moves.
int WordGenerate::roomLeftCount(int row, int col) {
  // count include the first char (itself).
  int countRoomLeft = 0;
  for (int offsetLeft = 0; offsetLeft <= currDirection;
       ++offsetLeft) {  // CHANGED
    if (board->hasaTile(row - cancelIndex * (offsetLeft),
                        col - (1 - cancelIndex) * (offsetLeft))) {
      countRoomLeft++;
    } else {
      break;
    }
  ...
  for (int offsetLeft = currDirection - countRoomLeft - 1; offsetLeft >= 0;
       --offsetLeft) {
    if (((*currCrossSet)[(1 - cancelIndex) * row + cancelIndex * (offsetLeft)]
                        [cancelIndex * col + (1 - cancelIndex) * (offsetLeft)]
                            .all()) &&
        !board->hasaTile(
            (1 - cancelIndex) * row + cancelIndex * ((offsetLeft)),
            cancelIndex * col + (1 - cancelIndex) * (offsetLeft))) {
      if (offsetLeft == 0) {
        countRoomLeft++;
      } else {
        if (!board->hasaTile(
                (1 - cancelIndex) * row + cancelIndex * (offsetLeft - 1),
                cancelIndex * col + (1 - cancelIndex) * (offsetLeft - 1))) {
          countRoomLeft++;
        }
      }
    } else {
      break;  // example: if you have a horizontally placed word "Log"  assume
              // there are 2 valid words called "report" and "elog" for
              // simplicity.
              /* 1-      *
                           R
                     _ _ _ _ L O G _ _ _
                  ...
                  2-
                     ...
                     _ _ _ _ _ L O G _ _ _
                      ...
                      So, in no. 1 here characters that would be taken into account are
                 the spaces after the intersection between the two words as the RoomLeft
                 = 6.
                      -> in the no. 2 example is "tricky" as the the anchor squares are
                 2 not one         to eliminate the duplicate words the RoomLeft of the
                 'G'         anchor would be 3 only.
                */
    }
  }
...
void WordGenerate::gen(int pos, string word, Node *gaddagNode) {
  ...
  if (board->hasaTile(anchorRow + (pos) * (cancelIndex),
                      anchorCol + (pos) * (1 - cancelIndex))) {
    boardLetter =
        board->getTileAtPosition(anchorRow + (pos) * (cancelIndex),
                                 anchorCol + (pos) * (1 - cancelIndex));
    ...
    while (childNode != NULL) {
      ...
      if ((tilesCount[childLetter - CHAR_OFFSET] > 0) &&
          (emptyBoard || (*currCrossSet)[anchorRow + (pos) * (cancelIndex)]
                                        [anchorCol + (pos) * (1 - cancelIndex)]
                                            .test(childLetter - CHAR_OFFSET))) {
        usedTiles++;  // counting tiles used.
        tilesCount[childLetter -
                   CHAR_OFFSET]--;  // meaning that a letter has been taken into
                                    // consideration dont repeat it.
    ...
    if (tilesCount[BLANK - CHAR_OFFSET] >
        0) {  // consider all chars if BLANK tile.
      ...
      while (childNode != NULL) {
        ...
        if ((childLetter != GADDAG_DELIMITER) &&
            (emptyBoard ||
             (*currCrossSet)[anchorRow + (pos) * (cancelIndex)]
                            [anchorCol + (pos) * (1 - cancelIndex)]
                                .test(childLetter - CHAR_OFFSET))) {
          usedTiles++;  // counting tiles used.
          tilesCount[BLANK -
                     CHAR_OFFSET]--;  // meaning that a letter has been taken
                                      // into consideration dont repeat it.
          childLetter =
              childLetter - BLANK_CHAR;  // to mark this letter as a BLANK USED.
          goOn(pos, childLetter, word, childNode);
...
// This Function it sets the Options needed for transforming from Horiz. to
...
void WordGenerate::setDirectionOptions(int row, int col, bool isHorizontal) {
  if (isHorizontal) {
    ...
    colOffset = 1;
    rowOffset = 0;
    currCrossSet = &Vertical_crossset;  // VerticalCrossSet;// dummy pass to it
                                        // Vericrossset[row][col] .
  } else {
    ...
    colOffset = 0;
    rowOffset = 1;
    currCrossSet = &Horiz_crossset;  // HorizCrossSet;// dummy pass to it
                                     // Horizcrossset[row][col] .
  }
}

// This Function GO ON funtion first function in Move Generation Algo.
void WordGenerate::goOn(int pos, char boardLetter, string word,
                        Node *gaddagNode) {
  // TODO: Complete The Algo. + Add any Helping Functions that may be Needed
  // During Implementation. cout<<"HI GADDAD"<<endl;
  if (pos <= 0) {
    bool roomLeft = true;
    ...
    if (((anchorCol + pos) * (1 - cancelIndex) +
         (anchorRow + pos) * (cancelIndex)) <= 0) {
      boardEdge = true;
      ...
      if (!board->hasaTile(anchorRow + pos * (cancelIndex),
                           anchorCol + pos * (1 - cancelIndex)) &&
          board->hasaTile(
              anchorRow + (pos - 1) * (cancelIndex),
              anchorCol +
                  (pos - 1) *
                      (1 - cancelIndex))) {  // does it have a letter left
                                             // (above) to this square
        roomLeft = false;
      }
    ...
    if (gaddagNode->isEndOfWord() && usedTiles != 0 &&
        (emptyBoard ||
         roomLeft))  // a move is FOUND if : end of a word is found (in GADDAG)
                     // and atleast a tile was used and.
    {
      ...
      newMove->moveScore =
          board->calculateScore(newMove->word, newMove->startPosition.ROW,
                                newMove->startPosition.COL,
                                newMove->horizontal) +
          (char)(bingoMove)*50;  // extra 10 points for now only.
    ...
    if (anchorRow + rowOffset >= maxBorder ||
        anchorCol + colOffset >= maxBorder) {
      rightBoardEdge = true;
    }  // board edge reached whehter from above or left.

    if (!rightBoardEdge && (emptyBoard || !directlyLeft)) {
      // RIGHT:
      Node *delimeterNode = gaddagNode->findChildChar(
          GADDAG_DELIMITER);  // search for the delimeter as on "newArc" to
                              // switch direction and  go right (SUFFIX).
    ...
    if (gaddagNode->isEndOfWord() && usedTiles != 0 &&
        (emptyBoard ||
         roomRight))  // a move is FOUND if : end of a word is found (in GADDAG)
                      // and atleast a tile was used and.
    {
      // create,calculate and add a new move in MOVE LIST.
      // using the class or struct(for now).
      ...
      newMove->word = word;
      newMove->horizontal = (cancelIndex == 0 ? true : false);
      newMove->startPosition.ROW =
          anchorRow + pos * (cancelIndex) + (1 - word.length()) * (cancelIndex);
      newMove->startPosition.COL = anchorCol + pos * (1 - cancelIndex) +
                                   (1 - word.length()) * (1 - cancelIndex);
...
void WordGenerate::crosssets() {
  ...
  for (int row = 0; row < MAX_BOARD_ROWS; row++) {
    for (int col = 0; col < MAX_BOARD_COLS; col++) {
      ...
      if (!board->hasaTile(row, col) && (col != 14) &&
        ...
        while (nod != 0) {
          ...
          nod = nod->getNextChild();
               ...
               (col == 14 || !board->hasaTile(row, col + 1)) && (col != 0) &&
               (board->hasaTile(row, col - 1))) {
        int move_col = col - 1;
        int ptr = 0;
        ...
        /**kh//kh*/
               ...
               (col == 0 || board->hasaTile(row, col - 1)) &&
               (col == 14 || board->hasaTile(row, col + 1))) {
        int R_move_col = col + 1;
        while (R_move_col < MAX_BOARD_COLS &&
               board->hasaTile(row, R_move_col++)) {
          ;
        }
        if (MAX_BOARD_COLS == R_move_col &&
            board->hasaTile(row, R_move_col - 1)) {
          R_move_col++;
        }
        R_move_col -= 2;
      ...
      // //////// Vertical
      if (!board->hasaTile(row, col) && (row != 14) &&
          (board->hasaTile(row + 1, col)) &&
          (row == 0 || !board->hasaTile(row - 1, col))) {
        int move_row = row + 1;
        ...
        while (nod != 0) {
          ...
          nod = nod->getNextChild();
               ...
               (row == 14 || !board->hasaTile(row + 1, col)) && (row != 0) &&
               (board->hasaTile(row - 1, col))) {
        int move_row = row - 1;
        move_row = row - 1;
        int ptr = 0;
        ...
        if (col == 3 && row == 2) {
          cout << "BELALVert 1" << endl;
               ...
               (row == 14 || board->hasaTile(row + 1, col)) &&
               (row == 0 || board->hasaTile(row - 1, col))) {
        int move_row = row + 1;
        while (move_row < MAX_BOARD_ROWS && board->hasaTile(move_row++, col)) {
        };
...
void WordGenerate::emptyBoardMoves() {
  if (!emptyBoard && false) {  // for now.
    ...
    for (std::list<string>::iterator it = nonRepeatedMoves.begin(); it != end;
         ++it) {
      for (unsigned int k = 0; k < (*it).length(); ++k) {
        ...
        move.moveScore =
            board->calculateScore(move.word, move.startPosition.ROW,
                                  move.startPosition.COL, move.horizontal) +
            (char)(bingoMove)*50;
...
}  // TODO: Generate all possible moves availabe given certain Rack when the
   // status of the board is empty only.

void WordGenerate::generateEmpty(Node *node, string &word) {
  for (Node *child = node->getFirstChild(); child;
       child = child->getNextChild()) {
    char childLetter = child->getNodeLetter();

    if (childLetter == GADDAG_DELIMITER ||
        tilesCount[childLetter - CHAR_OFFSET] <= 0) {
      continue;
    }
...
}  // TODO: removes duplicate moves occuring from a one tile play. (vertically +
   // Horizonatally).
  ...
  for (int row = 0; row < MAX_BOARD_ROWS; row++) {
    for (int col = 0; col < MAX_BOARD_COLS; col++) {
      if (!Horiz_crossset[row][col].all()) {
        cout << endl
             << "Horizontal Cross Set [" + to_string(row) + "]" + "[" +
                    to_string(col) + "] : "
             << Horiz_crossset[row][col] << endl;
      }

      if (!Vertical_crossset[row][col].all()) {
        cout << endl
             << "Vertical Cross Set [" + to_string(row) + "]" + "[" +
                    to_string(col) + "] : "
             << Vertical_crossset[row][col] << endl;
      }
    }
  }
}

// Updates CrossSets of The Board When a Move is Played. Given This 'Move' The
// Function Uses Move Specific Information And Updates .
void WordGenerate::updateCrossSet(Move *move) {
  ...
  int moveRow =
      move->startPosition.ROW + (move->word.length() - 1) * (1 - moveType);
  int moveCol =
      move->startPosition.COL + (move->word.length() - 1) * (moveType);

  if (moveRow >
      0) {  // Right and Left of a MOVE (Above or Below incase of Vertical Move)
