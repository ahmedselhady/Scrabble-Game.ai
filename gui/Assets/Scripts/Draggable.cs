    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (TileSelector.dropTargetType == DropTarget.Board) // Droping on board
        {
            ...
            if (startPositionBoardCoordinate.x > -240 && startPositionBoardCoordinate.x < 240 && startPositionBoardCoordinate.y > -240 && startPositionBoardCoordinate.y < 240)
            {
                ...
                if (GameManager.globalGameBoard.getBoard((int)(TileSelector.coordinateToIndex.x / 31.7f) + 7, (int)Mathf.Abs((TileSelector.coordinateToIndex.y / 31.7f) - 7)))
                {
                    ...
                    // TODO: setRack(index,null,true); get the index of the draggable tile from the rack
                    //GameManager.globalGameRack.setRack()
                    GameManager.globalGameBoard.setBoard((int)(TileSelector.coordinateToIndex.x / 31.7f) + 7, (int)Mathf.Abs((TileSelector.coordinateToIndex.y / 31.7f) - 7), this.gameObject, false);
                    GameManager.globalGameBoard.setBoard((int)(startPositionBoardCoordinate.x / 31.7f) + 7, (int)Mathf.Abs((startPositionBoardCoordinate.y / 31.7f) - 7), this.gameObject, true);
                }
                else
            ...
            else // Starting drag from the rack of tiles and dropping on the board
            {
                ...
                if (GameManager.globalGameBoard.getBoard((int)(TileSelector.coordinateToIndex.x / 31.7f) + 7, Mathf.Abs((int)(TileSelector.coordinateToIndex.y / 31.7f) - 7)))
                {
